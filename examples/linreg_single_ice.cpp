...
int main()
{
    ...
    // Regression: one feature
    // Data: https://www.kaggle.com/datasets/raphaelmanayon/temperature-and-ice-cream-sales
    Features data;
    data.load("./data/Ice Cream Sales - temperatures.csv", arma::csv_ascii);
    // Skip the header
    data.shed_row(0);
    Features X = data.cols(0, 0);
    X.brief_print("\nFeatures: Temperature");
    Target y = data.cols(1, 1);
    y.brief_print("\nTarget: Ice Cream Sales");

    // Fit linear regression model with either OLS or derivative solver
    
    //OLSSolver solver;
    //QRSolver solver;
    double lr = 0.8; // 5e-7
    size_t max_iter = 10000; // 5e5
    double max_deriv_size = 1e-6; // 3.0
    bool verbose = false;
    // MSE grad
